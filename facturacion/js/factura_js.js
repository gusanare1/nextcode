...
app.controller('consulta_facturas', function($scope, $http, $window, $timeout,service_atras,$filter) {
var dateE= new Date();
$scope.date2 = dateE;
...
$filter.$myfilter;
	...
	$http({
		  method: 'POST',
		  url: '../busquedas.php',
		  data: {'action':'consulta_factura'}

		}).then(function successCallback(response) {
			console.log(response.data);
			$scope.orderProperty = "fecha_emision";
...
app.filter("myfilter", function() {
  return function(items, from, to) {
		...
		var df= from.getFullYear() + "-" + (from.getMonth() + 1) + "-" + (from.getDate());
		var dt= to.getFullYear() + "-" + (to.getMonth() + 1) + "-" + to.getDate();
		
		var startdate = new Date(df);
		...
		for(i=0;i<items.length;i++)
		{
			...
			var fe = items[i].fecha_emision;
			var de = new Date(fe);
			//if( df < fe && dt > fe)
			
			if(de.getTime()>=startdate.getTime() && de.getTime()<=enddate.getTime()) //fe between startdate
				result.push(items[i]);
		}
